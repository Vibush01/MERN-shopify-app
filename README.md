...
Built as part of the FutureBlink assessment task.
...
3. Behind the scenes, the app:
   ...
   - Updates a **Shop Metafield** on Shopify via the GraphQL Admin API, making the text available to the storefront.
...
5. The admin dashboard also shows a **Modification Audit Trail** table — a history of every announcement that was ever set, pulled from MongoDB.
...
The Shopify-related env variables (`SHOPIFY_API_KEY`, `SHOPIFY_API_SECRET`, etc.) are automatically injected by the Shopify CLI when you run `npm run dev`, so you don't need to add those manually.
...
Now go back to your app in the Shopify Admin, type an announcement, and hit save. The banner should appear on your storefront.
...
4. **Sets the metafield** — calls `metafieldsSet` with the shop GID as the owner, writing to `my_app.announcement`.
...
It's configured as an **App Embed** (target: `body`), so it doesn't require merchants to manually place it in their theme. They just toggle it on.
...
`storefront = "public_read"` is what makes the metafield readable in Liquid templates.
